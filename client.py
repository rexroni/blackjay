...
def synchronize(force_pull=False):
    # debounce timeout
    sleep(.3)
    global global_ip, global_port, global_config
    ...
    push = add_hmacs_to_metadata(push, global_config['password'])
    prep_client_to_server_archive(push, pull, conflicts, global_config['password'])
    ...
    make_client_updates_live(npush,npull,nconfl,global_config['password'])
...
if __name__ == "__main__":
    ...
    tunnel = None if global_config['host'] == 'localhost' \
                  else sshtunnel.SSHTunnelForwarder(global_config['host'],
                       remote_bind_address=('localhost',int(global_config['port'])))

    if global_config['host'] != 'localhost':
        ...
        global_ip = '127.0.0.1'
        global_port = int(global_config['port'])
