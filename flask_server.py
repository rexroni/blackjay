...
def extract_client_to_server_archive(base_dir, zipfile, UID):
    ...
    tempdir = os.path.join(root_dir, base_dir, '.blackjay/c2s'+UID)
    os.mkdir(tempdir)
    # extract the zip file
    with ZipFile(zipfile,'r') as z:
        z.extractall(tempdir)
    push = load_metadata(os.path.join(root_dir, tempdir,'.blackjay/push'))
    pull = load_metadata(os.path.join(root_dir, tempdir,'.blackjay/pull'))
    conflicts = load_metadata(os.path.join(root_dir, tempdir,'.blackjay/conflicts'))
...
def prep_server_to_client_archive(base_dir, push, pull, conflicts, UID):
    ...
    zipname = os.path.join(root_dir, base_dir, '.blackjay/s2c'+UID+'.zip')
    # now prep the zip archive
    with ZipFile(zipname,'w') as z:
        z.writestr(os.path.join(root_dir, base_dir,'.blackjay/push'),json.dumps(push,indent=3))
        z.writestr(os.path.join(root_dir, base_dir,'.blackjay/pull'),json.dumps(pull,indent=3))
        z.writestr(os.path.join(root_dir, base_dir,'.blackjay/conflicts'),json.dumps(conflicts,indent=3))
        for name,meta in pull.items():
            if meta['del_flag'] is False:
                z.write(os.path.join(root_dir, base_dir, name))
...
def make_server_updates_live(base_dir, push,UID):
    meta_filename = os.path.join(root_dir, base_dir, '.blackjay/metadata')
    ...
    for name,meta in push.items():
        ...
        dest_name = os.path.join(root_dir, base_dir, name)
        source_name = os.path.join(root_dir, base_dir, '.blackjay/c2s'+UID, name)
        if meta['del_flag'] is False:
...
def cleanup_server_temp_files(base_dir, UID):
    os.remove(os.path.join(root_dir, base_dir, '.blackjay/c2s'+UID+'.zip'))
    os.remove(os.path.join(root_dir, base_dir, '.blackjay/s2c'+UID+'.zip'))
    shutil.rmtree(os.path.join(root_dir, base_dir, '.blackjay/c2s'+UID))
...
def get_metadata(base_dir):
    ...
    blackjay_path = os.path.join(root_dir, base_dir,'.blackjay')
    if os.path.isdir(os.path.join(root_dir, base_dir, '.blackjay')) is False:
    ...
    os.mkdir(os.path.join(root_dir, blackjay_path,uid))
    # archive the local metadata folder for future comparisons
    # MUTEX LOCK
    shutil.copy(os.path.join(root_dir, blackjay_path,'metadata'),
                os.path.join(root_dir, blackjay_path,uid,'metadata'))
    # MUTEX UNLOCK
    meta = load_metadata(os.path.join(root_dir, blackjay_path,uid,'metadata'))
...
def post_update(base_dir,uid):
    ...
    if '.blackjay/c2s.zip' not in request.files:
        ...
        abort(400)
    if not os.path.isdir(os.path.join(root_dir, base_dir, '.blackjay')):
        #abort(404, message="Base Directory {} doesn't exist".format(base_dir))
        abort(404)
    req_zipname = os.path.join(root_dir, base_dir, '.blackjay/c2s{}.zip'.format(uid))
